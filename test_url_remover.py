...
class TestURLRemover(unittest.IsolatedAsyncioTestCase):
    ...
    def _set_settings(self, **kwargs):
        ...
        try:
            s = session.query(URLRemoverSettings).filter(URLRemoverSettings.chat_id == self.chat_id).first()
            if not s:
    ...
    async def test_contains_url_detection(self):
        from handlers.url_remover import contains_invite_link, contains_url
    ...
    async def test_remove_urls_deletes_message(self):
        ...
        ctx.bot.id = 1
        with patch("database.db.is_admin", return_value=False), patch("database.db.is_whitelisted", return_value=False):
    ...
    async def test_remove_urls_skips_admins(self):
        ...
        ctx = MagicMock()
        with patch("database.db.is_admin", return_value=True), patch("database.db.is_whitelisted", return_value=False):
            result = await check_url_remover(update, ctx)
