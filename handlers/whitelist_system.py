...
from utils import format_user_mention, get_user_from_message, is_admin_command, is_group_command
...
async def whitelist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to whitelist (reply to message, @username, or user ID)."
        )
        return
...
async def gwhitelist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to globally whitelist (reply to message, @username, or user ID)."
        )
        return
...
async def unwhitelist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to remove from whitelist (reply to message, @username, or user ID)."
        )
        return
    ...
    if user_id:
        if db.remove_whitelist(user_id, chat_id):
            user_mention = format_user_mention(user_obj) if user_obj else f"User {user_id}"
            await update.message.reply_text(
                f"✅ {user_mention} has been removed from the whitelist!", parse_mode="Markdown"
            )
        else:
...
async def gunwhitelist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to remove from global whitelist (reply to message, @username, or user ID)."
        )
        return
    ...
    if user_id:
        if db.remove_whitelist(user_id, is_global=True):
            user_mention = format_user_mention(user_obj) if user_obj else f"User {user_id}"
            await update.message.reply_text(
                f"🌐 {user_mention} has been removed from the global whitelist!", parse_mode="Markdown"
            )
        else:
...
async def whitelisted_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        whitelisted = (
            ...
            .order_by(db.Whitelist.created_at.desc())
            .limit(20)
            .all()
        )
...
async def checkwhitelist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to check whitelist status (reply to message, @username, or user ID)."
        )
        return
    ...
    if user_id:
        ...
        if is_whitelisted:
            ...
            try:
                global_whitelist = (
                    session.query(db.Whitelist)
                    .filter(db.Whitelist.user_id == user_id, db.Whitelist.is_global == True)
                    .first()
                )

                local_whitelist = (
                    session.query(db.Whitelist)
                    .filter(db.Whitelist.user_id == user_id, db.Whitelist.chat_id == chat_id)
                    .first()
                )
            ...
            await update.message.reply_text(f"❌ {user_mention} is not whitelisted.", parse_mode="Markdown")
