...
async def warnmode_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode not in VALID_WARN_MODES:
        await update.message.reply_text(f"❌ Invalid warn mode. Use: {', '.join(VALID_WARN_MODES)}")
        return
    ...
    try:
        ...
        try:
            ...
            if chat and chat.pinned_message_id:
                await context.bot.unpin_chat_message(chat_id=chat_id, message_id=chat.pinned_message_id)
...
async def purge_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if deleted_count > 0:
            confirmation = await context.bot.send_message(chat_id, f"🗑️ Purged {deleted_count} messages.")

            # Schedule deletion of confirmation message
            context.job_queue.run_once(lambda context: context.bot.delete_message(chat_id, confirmation.message_id), 5)
...
async def spurge_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].isdigit():
        ...
        messages_to_delete = list(range(start_id, current_message_id))
        if len(messages_to_delete) > Config.PURGE_LIMIT:
            await update.message.reply_text(f"❌ Too many messages to delete (max {Config.PURGE_LIMIT}).")
            return
    else:
        return
