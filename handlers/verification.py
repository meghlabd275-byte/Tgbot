...
async def verify_admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        admin_chats = session.query(db.Admin).filter(db.Admin.user_id == forwarded_user.id).all()
        ...
        verification_msg = "✅ **Verification Result: VERIFIED ADMIN**\n\n"
        ...
        verification_msg += "🛡️ **Admin Status Confirmed**\n"
        ...
        verification_msg += "⚠️ **Security Note:** This verification confirms the user is an admin "
        verification_msg += "in chats where this bot is present. Always verify through official channels "
        verification_msg += "for important matters."

        await message.reply_text(verification_msg, parse_mode="Markdown")

        # Log the verification attempt
        logger.info(f"Admin verification: User {verifier_user.id} verified admin status of user {forwarded_user.id}")
