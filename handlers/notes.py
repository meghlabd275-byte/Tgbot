...
async def save_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        existing_note = session.query(Note).filter(Note.chat_id == chat_id, Note.name == note_name).first()
...
async def get_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    session = db.get_session()
    try:
        note = session.query(Note).filter(Note.chat_id == chat_id, Note.name == note_name).first()
        ...
        if note.file_id and note.file_type:
            ...
            if note.file_type == "photo":
                await context.bot.send_photo(chat_id, note.file_id, caption=note.content, parse_mode="Markdown")
            elif note.file_type == "video":
                await context.bot.send_video(chat_id, note.file_id, caption=note.content, parse_mode="Markdown")
            elif note.file_type == "document":
                await context.bot.send_document(chat_id, note.file_id, caption=note.content, parse_mode="Markdown")
                ...
                await context.bot.send_animation(chat_id, note.file_id, caption=note.content, parse_mode="Markdown")
            elif note.file_type == "audio":
                await context.bot.send_audio(chat_id, note.file_id, caption=note.content, parse_mode="Markdown")
        else:
            # Send text note
            await context.bot.send_message(chat_id, note.content, parse_mode="Markdown")
...
async def setrules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if existing_rules:
            ...
            rules = Rule(chat_id=chat_id, content=rules_text, created_by=admin_id)
            session.add(rules)
            session.commit()
                ...
                if note.file_id and note.file_type:
                    if note.file_type == "photo":
                        ...
                        await context.bot.send_document(
                            chat_id, note.file_id, caption=note.content, parse_mode="Markdown"
                        )
                    elif note.file_type == "sticker":
                        await context.bot.send_sticker(chat_id, note.file_id)
                        if note.content:
