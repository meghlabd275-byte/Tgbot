...
class URLRemoverSettings(Base):
    ...
    remove_all_links = Column(
        Boolean, default=False
    )  # delete everything link-like (urls + invites + @mentions-as-links)
...
def message_has_link(message) -> bool:
    ...
    return any(contains_url(url) or contains_invite_link(url) for url in _extract_entity_urls(message))
...
async def check_url_remover(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        if s.remove_all_links:
            ...
            if (
                not should_delete
                and s.remove_invites
                and (contains_invite_link(text) or any(contains_invite_link(u) for u in _extract_entity_urls(message)))
            ):
                should_delete = True
        ...
        if s.warn_user:
            try:
                ...
                await context.bot.send_message(
                    chat_id, f"⚠️ {update.effective_user.first_name}, links are not allowed here. Warning {count}/3"
                )
            except Exception as e:
