...
async def promote_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to promote (reply to message, @username, or user ID)."
        )
        return
...
async def title_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id:
        try:
            await context.bot.set_chat_administrator_custom_title(chat_id=chat_id, user_id=user_id, custom_title=title)

            # Update in database
            session = db.get_session()
            try:
                admin = session.query(db.Admin).filter(db.Admin.user_id == user_id, db.Admin.chat_id == chat_id).first()
                if admin:
                    admin.title = title
                    session.commit()
            finally:
    ...
    if user_id:
        ...
        if db.remove_admin(user_id, chat_id):
            ...
            try:
                ...
                user_mention = format_user_mention(user_obj) if user_obj else f"User {user_id}"
                await update.message.reply_text(
                    f"✅ {user_mention} has been demoted from admin!", parse_mode="Markdown"
                )
            except BadRequest as e:
...
async def gban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to globally ban (reply to message, @username, or user ID)."
        )
        return
...
async def gunban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to globally unban (reply to message, @username, or user ID)."
        )
        return
...
async def banlist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        bans = (
            ...
            .filter((db.Ban.chat_id == chat_id) | (db.Ban.is_global == True))
            .order_by(db.Ban.created_at.desc())
            .limit(20)
            .all()
        )
...
async def gkick_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to globally kick (reply to message, @username, or user ID)."
        )
        return
