...
from utils import format_user_mention, get_user_from_message, is_admin_command
...
async def resetuser_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id:
        ...
        try:
            # Remove bans
            bans_removed = session.query(db.Ban).filter(db.Ban.user_id == user_id, db.Ban.chat_id == chat_id).delete()

            # Remove warnings
            warnings_removed = (
                session.query(db.Warning).filter(db.Warning.user_id == user_id, db.Warning.chat_id == chat_id).delete()
            )

            # Remove mutes
            mutes_removed = (
                session.query(db.Mute).filter(db.Mute.user_id == user_id, db.Mute.chat_id == chat_id).delete()
            )
...
async def resetrep_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to reset reputation for (reply to message, @username, or user ID)."
        )
        return
...
async def user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id:
        ...
        try:
            ...
            # Get user stats
            warning_count = (
                session.query(db.Warning)
                .filter(db.Warning.user_id == user_id, (db.Warning.chat_id == chat_id) | (db.Warning.is_global == True))
                .count()
            )

            is_banned = (
                session.query(db.Ban)
                .filter(db.Ban.user_id == user_id, (db.Ban.chat_id == chat_id) | (db.Ban.is_global == True))
                .first()
                is not None
            )

            is_muted = (
                session.query(db.Mute)
                .filter(db.Mute.user_id == user_id, db.Mute.chat_id == chat_id, db.Mute.until > datetime.now())
                .first()
                is not None
            )
            ...
            user_info_msg = "👤 **User Information**\n\n"
...
async def lastactive_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to check last active date (reply to message, @username, or user ID)."
        )
        return
    ...
    if user_id:
        ...
        try:
            ...
            if user and user.last_active:
                ...
                await update.message.reply_text(f"❌ No activity data found for {user_mention}", parse_mode="Markdown")
        finally:
            session.close()
...
async def chatinfo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Count banned users
        ban_count = session.query(db.Ban).filter((db.Ban.chat_id == chat.id) | (db.Ban.is_global == True)).count()

        chat_info_msg = "💬 **Chat Information**\n\n"
