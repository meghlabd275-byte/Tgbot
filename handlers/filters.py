...
SUSPICIOUS_DOMAINS = ["bit.ly", "tinyurl.com", "short.link", "t.co", "goo.gl", "ow.ly", "buff.ly", "is.gd", "tiny.cc"]
...
async def addfilter_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        existing = session.query(WordFilter).filter(WordFilter.chat_id == chat_id, WordFilter.word == word).first()
        ...
        if existing:
            ...
            word_filter = WordFilter(chat_id=chat_id, word=word, action=action, created_by=admin_id)
...
async def removefilter_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    session = db.get_session()
    try:
        word_filter = session.query(WordFilter).filter(WordFilter.chat_id == chat_id, WordFilter.word == word).first()
...
async def lock_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        for media_type in requested:
            existing = (
                session.query(MediaFilter)
                .filter(MediaFilter.chat_id == chat_id, MediaFilter.media_type == media_type)
                .first()
            )
            ...
            if existing:
                ...
                session.add(MediaFilter(chat_id=chat_id, media_type=media_type, is_locked=True, created_by=admin_id))
        session.commit()
...
async def locks_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    session = db.get_session()
    try:
        rows = session.query(MediaFilter).filter(MediaFilter.chat_id == chat_id).all()
...
async def allowlist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        existing = (
            session.query(URLAllowlist).filter(URLAllowlist.chat_id == chat_id, URLAllowlist.domain == domain).first()
        )
...
async def unallowlist_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    session = db.get_session()
    try:
        row = session.query(URLAllowlist).filter(URLAllowlist.chat_id == chat_id, URLAllowlist.domain == domain).first()
        if row:
            session.delete(row)
            session.commit()
...
async def antispam_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if settings:
            ...
            session.commit()
        await update.message.reply_text(f"✅ Anti-spam protection {'enabled' if status else 'disabled'}.")
    finally:
        session.close()
...
async def check_message_filters(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # Check word filters
    if text_to_check and await check_word_filters(update, context):
        return True

    # Check URL filters (domain-based blocklist)
    if (
        text_to_check
        and ("http" in text_to_check or "www." in text_to_check or "t.me" in text_to_check)
        and await check_url_filters(update, context)
    ):
        return True
    ...
    return bool(text_to_check and is_antispam_enabled(chat_id) and await check_spam_patterns(update, context))
...
async def check_media_filters(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    def _locked(session, t):
        return (
            session.query(MediaFilter)
            .filter(MediaFilter.chat_id == chat_id, MediaFilter.media_type == t, MediaFilter.is_locked == True)
            .first()
            is not None
        )
    ...
    try:
        ...
        if text_for_url:
            ...
            if _locked(session, "url") and ("http" in text_for_url or "www." in text_for_url):
                urls = re.findall(
                    r"http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+", text_for_url
                )
    ...
    try:
        ...
        if action == "warn":
            ...
            action_msg = await context.bot.send_message(
                chat_id,
                f"⚠️ {user.first_name} warned for: {reason}\nWarnings: {warning_count}/{settings['limit']}{extra}",
            )
            ...
            action_msg = await context.bot.send_message(chat_id, f"🔇 {user.first_name} muted for 1 hour: {reason}")

        elif action == "kick":
            await context.bot.ban_chat_member(chat_id, user_id)
            await context.bot.unban_chat_member(chat_id, user_id)

            action_msg = await context.bot.send_message(chat_id, f"👢 {user.first_name} kicked: {reason}")

        elif action == "ban":
            await context.bot.ban_chat_member(chat_id, user_id)
            db.add_ban(user_id, chat_id, context.bot.id, reason)

            action_msg = await context.bot.send_message(chat_id, f"🔨 {user.first_name} banned: {reason}")

        # Delete action message after 5 seconds (except for delete-only)
        if action != "delete":
            context.job_queue.run_once(lambda context: context.bot.delete_message(chat_id, action_msg.message_id), 5)
