...
• New users are auto-muted"""
        ...
        try:
            ...
            if settings:
                ...
                settings = ChatSettings(chat_id=chat_id, night_mode_start=start_time, night_mode_end=end_time)
                session.add(settings)
                session.commit()
...
async def slowmode_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args[0].lower() == "status":
        ...
        try:
            ...
            await update.message.reply_text(
                f"🐌 **Slow Mode:** Enabled\n**Delay:** {settings.slow_mode_delay} seconds", parse_mode="Markdown"
            )
        ...
        try:
            ...
            if settings:
                ...
                settings = ChatSettings(chat_id=chat_id, slow_mode_enabled=True, slow_mode_delay=delay)
                session.add(settings)
                session.commit()
...
async def addcmd_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        existing = (
            ...
            .filter(CustomCommand.chat_id == chat_id, CustomCommand.command == command)
            .first()
        )
        ...
        if existing:
            ...
            custom_cmd = CustomCommand(chat_id=chat_id, command=command, response=response, created_by=admin_id)
...
async def cleanup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Get inactive users
        inactive_users = session.query(db.User).filter(db.User.last_active < cutoff_date).all()
...
async def backup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from handlers.filters import MediaFilter, URLAllowlist, URLFilter, WordFilter
            ...
            try:
                ...
                if warning_key not in night_mode_restrictions:
                    ...
                    context.job_queue.run_once(
                        lambda context: context.bot.delete_message(chat_id, warning_msg.message_id), 10
                    )
