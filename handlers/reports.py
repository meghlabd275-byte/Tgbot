...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "❌ Please reply to a message to report it.\nUsage: `/report [reason]`", parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        if cooldown_key in report_cooldowns:
            ...
            if time_diff < cooldown_time:
                ...
                await update.message.reply_text(f"⏰ Please wait {remaining} seconds before reporting again.")
                return
...
async def send_report_to_admins(
    context: ContextTypes.DEFAULT_TYPE, report: Report, reported_message, chat, settings=None
):
    ...
    try:
        ...
        if settings is None:
            settings = session.query(ReportSettings).filter(ReportSettings.chat_id == report.chat_id).first()
...
{reported_message.text or "[Media/Sticker/Other]"}"""
        ...
        if not admin_only:
            try:
                await context.bot.send_message(
                    report.chat_id, report_text, parse_mode="Markdown", reply_markup=keyboard
                )
        ...
        for admin in admins:
            try:
                await context.bot.send_message(admin.user_id, report_text, parse_mode="Markdown", reply_markup=keyboard)
            except Exception:
...
async def handle_report_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            report.status = (
                "resolved" if action in ["ban", "kick", "mute", "warn", "delete", "resolve"] else "dismissed"
            )
            ...
            await query.edit_message_text(
                f"{query.message.text}\n\n❌ **Failed to {action}: {e!s}**", parse_mode="Markdown"
            )
...
async def reports_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        try:
            ...
            pending_reports = (
                session.query(Report).filter(Report.chat_id == chat_id, Report.status == "pending").count()
            )
...
• `/reporthistory` - View report history"""
    ...
    if context.args[0].lower() in ["on", "off"]:
        ...
        try:
            ...
            await update.message.reply_text(f"✅ Admin only reports {'enabled' if status else 'disabled'}.")
        ...
        try:
            ...
            await update.message.reply_text(f"✅ Auto-delete report commands {'enabled' if status else 'disabled'}.")
...
async def reporthistory_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        reports = (
            session.query(Report).filter(Report.chat_id == chat_id).order_by(Report.created_at.desc()).limit(10).all()
        )
        ...
        for report in reports:
            status_emoji = {"pending": "⏳", "resolved": "✅", "dismissed": "❌"}.get(report.status, "❓")
