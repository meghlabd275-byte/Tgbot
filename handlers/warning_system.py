...
async def gwarn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to globally warn (reply to message, @username, or user ID)."
        )
        return
    ...
    if user_id:
        ...
        try:
            total_warnings = session.query(db.Warning).filter(db.Warning.user_id == user_id).count()
        finally:
            session.close()
...
async def unwarn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to remove warning from (reply to message, @username, or user ID)."
        )
        return
...
async def resetwarns_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to reset warnings for (reply to message, @username, or user ID)."
        )
        return
...
async def warnings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_info:
        await update.message.reply_text(
            "❌ Please specify a user to check warnings for (reply to message, @username, or user ID)."
        )
        return
    ...
    if user_id:
        ...
        try:
            recent_warnings = (
                ...
                .order_by(db.Warning.created_at.desc())
                .limit(5)
                .all()
            )
