...
def test_register_and_query_clone():
    row, created = db.register_bot_instance("111:AAA", "clone_one", 111, created_by=777000, status="active")
...
def test_fleet_membership_add_leave():
    ...
    assert set(groups[0]["bot_ids"]) == {111, 0, 222} if False else set(groups[0]["bot_ids"]) >= {111, 0}
...
def test_clone_supervisor_lifecycle(monkeypatch):
    ...
    row, _ = db.register_bot_instance("989898:TEST", "test_clone", 989, status="active")
...
def test_clone_registration_then_groups_listing(monkeypatch):
    ...
    row, _ = db.register_bot_instance("121212:CLONE", "fleet_helper", 121, status="active")
    ...
    target = next(g for g in groups if g["chat_id"] == -200200)
